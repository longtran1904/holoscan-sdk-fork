...
scheduler single-threaded. The trials use 1, 2, 4, 8, 12, 14, and 16 operators, each executing
...
```bash
./examples/benchmark_scheduler_throughput_greedy/cpp/benchmark_scheduler_throughput_greedy
```
...
```bash
./examples/benchmark_scheduler_throughput_greedy/cpp/benchmark_scheduler_throughput_greedy --busy_wait
```

This mode uses 1,000 operations per operator, so the full sweep takes at least 57 seconds.
...
Without this flag, the benchmark uses `CountOp` and 10,000,000 operations per operator.
