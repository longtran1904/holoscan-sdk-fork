...
template <typename T> static T ceildiv(T x, size_t divisor) {
...
template <> struct StorageType<__nv_fp4_e2m1> {
    static constexpr size_t packing = 2;
    using type = __nv_fp4x2_e2m1;
};

template <typename T> constexpr size_t sizeofElements(size_t N) {
    return ceildiv(N, StorageType<T>::packing);
}

template <typename T> constexpr size_t sizeofBytes(size_t N) {
...
template <typename T> static bool isNarrowPrecision() {
    return std::is_same<T, __nv_fp8_e4m3>::value || std::is_same<T, __nv_fp8_e5m2>::value ||
           std::is_same<T, __nv_fp4_e2m1>::value;
}

template <typename InTypeAB,
          typename OutType = InTypeAB,
          typename ComputeType = OutType,
          typename ScaleType = ComputeType,
          typename DScaleType = ScaleType,
          typename InTypeC = OutType>
struct TestBench {
    ...
    TestBench(cublasOperation_t transa,
              ...
              int n,
              int k,
              ComputeType alpha = ComputeType{1.0f},
    ...
    TestBench(cublasOperation_t transa,
        ...
        checkCudaStatus(
            cudaMalloc(reinterpret_cast<void **>(&DOutscaleDev), DOutscaleHost.size() * sizeof(DOutscaleHost[0])));
    ...
    void copyDataToDevice() {
        ...
        if (isPtrArray(ptrArrayBatch, groupedBatch)) {
            ...
            for (int i = 0; i < N; ++i) {
                checkCudaStatus(cudaMemcpyAsync(APtrArrayHost[i], &Ahost[i * m * n],
        ...
        checkCudaStatus(cudaMemcpyAsync(BscaleDev, BscaleHost.data(), BscaleHost.size() * sizeof(BscaleHost[0]),
                                        cudaMemcpyHostToDevice, stream));
        checkCudaStatus(cudaMemcpyAsync(CscaleDev, CscaleHost.data(), CscaleHost.size() * sizeof(CscaleHost[0]),
                                        cudaMemcpyHostToDevice, stream));
        checkCudaStatus(cudaMemcpyAsync(DscaleDev, DscaleHost.data(), DscaleHost.size() * sizeof(DscaleHost[0]),
                                        cudaMemcpyHostToDevice, stream));
            ...
            for (int i = 0; i < N; ++i) {
                if (outOfPlace)
                    checkCudaStatus(cudaMemcpyAsync(&Dhost[i * m * n], DPtrArrayHost[i],
                                                    Dhost.size() / N * sizeof(Dhost[0]), cudaMemcpyDeviceToHost,
                                                    stream));
                else
                    checkCudaStatus(cudaMemcpyAsync(&Chost[i * m * n], CPtrArrayHost[i],
                                                    Chost.size() / N * sizeof(Chost[0]), cudaMemcpyDeviceToHost,
                                                    stream));
            }
        } else {
            if (outOfPlace)
                checkCudaStatus(cudaMemcpyAsync(Dhost.data(), Ddev, Dhost.size() * sizeof(Dhost[0]),
                                                cudaMemcpyDeviceToHost, stream));
            else
                checkCudaStatus(cudaMemcpyAsync(Chost.data(), Cdev, Chost.size() * sizeof(Chost[0]),
                                                cudaMemcpyDeviceToHost, stream));
        }

        checkCudaStatus(cudaMemcpyAsync(DOutscaleHost.data(), DOutscaleDev,
                                        DOutscaleHost.size() * sizeof(DOutscaleHost[0]), cudaMemcpyDeviceToHost,
                                        stream));
    }
...
template <> inline void TestBench<__half, __half, cuComplex>::fillData() {
